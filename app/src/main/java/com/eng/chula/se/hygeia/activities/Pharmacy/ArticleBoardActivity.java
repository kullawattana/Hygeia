...
public class ArticleBoardActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    final String ArticleBoardActivity = "ArticleBoardActivity";

    Integer writerIdToInt = 0;
    ...
    private void articleBoard(View v) {
        Integer articleId = 1;
        String articleName = editTextArticleName.getText().toString().trim();
        String writerId = editTextWriterId.getText().toString();
        ...
        if((!articleName.isEmpty() && !writerId.isEmpty() && !articleContent.isEmpty())){

            openDialog(v);

            SharedPreferences.Editor editor = getSharedPreferences(ArticleBoardActivity, MODE_PRIVATE).edit();
            writerIdToInt = Integer.valueOf(writerId);
            editor.putInt("articleId", articleId);
            editor.putString("articleName", articleName);
            editor.putInt("writerId", writerIdToInt);
            editor.putString("writerId", articleContent);
            editor.apply();

            Call<DefaultResponse> call = RetrofitClient
                    .getInstance()
                    .getApi()
                    .articleBoard(articleId, articleName, writerIdToInt, articleContent);
